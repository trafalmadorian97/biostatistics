from src_new.assets.executable.extracted.magma_binary_extracted import (
    MAGMA_1_1_BINARY_EXTRACTED,
)
from src_new.assets.gwas.me_cfs.decode_me.processed_gwas_data.decode_me_gwas_1_build_37_magma_snp_locs import (
    DECODE_ME_GWAS_1_BUILD_37_MAGMA_SNP_LOCS,
)
from src_new.assets.reference_data.magma_gene_locations.processed.magma_gene_locations_data_build_37_unzipped import (
    MAGMA_GENE_LOCATION_REFERENCE_DATA_BUILD_37_EXTRACTED,
)
from src_new.build_system.task.magma_annotate_task import MagmaAnnotateTask

DECODE_ME_GWAS_1_MAGMA_ANNOTATIONS = MagmaAnnotateTask.create(
    asset_id="decode_me_gwas_1_build_37_magma_annotations",
    magma_binary_task=MAGMA_1_1_BINARY_EXTRACTED,
    snp_loc_file_task=DECODE_ME_GWAS_1_BUILD_37_MAGMA_SNP_LOCS,
    gene_loc_file_task=MAGMA_GENE_LOCATION_REFERENCE_DATA_BUILD_37_EXTRACTED,
)
